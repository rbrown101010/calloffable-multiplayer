...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='status'||action==='start'||action==='end'){
      ...
      if(!access.id||access.expiresAt<Date.now()||!current?.active||current.protocol!==7||current.roomId!==access.roomId)return res.status(410).json({error:'This lobby has ended. Ask the owner for the new invitation.'});
      ...
      const presence=await db.rooms.getPresence('sable',current.roomId+'-match-v7');
    ...
    if(action==='join'&&(!current?.active||current.protocol!==7||!equal(String(body.key||''),current.inviteKey)))return res.status(403).json({error:'That invitation is no longer active. Ask the owner for the new lobby link.'});
    ...
    if(action==='create'){
      ...
      if(!body.requestId||body.requestId!==current?.requestId||current.ownerId!==userId||!current.active){
        current={protocol:7,roomId:randomUUID(),ownerId:userId,inviteKey:randomBytes(18).toString('base64url'),active:true,requestId:String(body.requestId||randomUUID()),createdAt:Date.now()};
