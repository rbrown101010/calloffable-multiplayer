...
try{
 ...
 const images=await page.evaluate(async()=>{
  const {THREE}=window.review,{WEAPONS}=await import('/src/game/WeaponDefs.ts'),{loadWeaponModel,centerModel}=await import('/src/game/Weapons.ts');
  ...
  const result={};for(const id of Object.keys(WEAPONS)){
   const def=WEAPONS[id],model=await loadWeaponModel(def);model.rotation.set(...def.model.rot);model.scale.setScalar(def.model.scale);centerModel(model);scene.add(model);const optic=new THREE.Group(),mat=new THREE.MeshStandardMaterial({color:0x1b292b,roughness:.4,metalness:.7});if(def.optic){if(def.optic==='holographic'){for(const x of[-.049,.049]){const m=new THREE.Mesh(new THREE.BoxGeometry(.008,.079,.028),mat);m.position.x=x;optic.add(m);}for(const y of[-.043,.043]){const m=new THREE.Mesh(new THREE.BoxGeometry(.1,.008,.045),mat);m.position.y=y;optic.add(m);}}else optic.add(new THREE.Mesh(new THREE.TorusGeometry(.045,.005,6,24),mat));const mount=new THREE.Mesh(new THREE.BoxGeometry(.055,.04,.07),mat);mount.position.y=-.058;optic.add(mount);optic.position.y=new THREE.Box3().setFromObject(model).max.y+.06;scene.add(optic);}
