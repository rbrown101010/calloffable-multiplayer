...
try{
 ...
 const images=await page.evaluate(async()=>{
  ...
  const result={};for(const id of ['scarh','intervention','ak47','mp5','spas12','deagle','m1911']){
   const def=WEAPONS[id],model=await loadWeaponModel(def);model.rotation.set(...def.model.rot);model.scale.setScalar(def.model.scale);centerModel(model);scene.add(model);
   ...
   const distance=Math.max(5,size.length()*3);const camera=new THREE.OrthographicCamera(-height*3.0476/2,height*3.0476/2,height/2,-height/2,.01,distance*4);camera.position.set(distance,distance*.15,distance*.1);camera.lookAt(0,0,0);renderer.render(scene,camera);result[id]=renderer.domElement.toDataURL('image/png').split(',')[1];scene.remove(model);
